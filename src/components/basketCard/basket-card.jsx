...
export const BasketCard = ({ img, title, id, price, count, userCount }) => {
  const dispatch = useDispatch();
  const [showBtn, setShowBtn] = React.useState(false);

  const plusFn = () => {
    if (userCount < count) {
      dispatch(toggleAmount({ type: "plus", id }));
    } else {
      setShowBtn(true);
    }
  };
  ...
  const minusFn = () => {
    if (userCount > 1) {
      dispatch(toggleAmount({ type: "minus", id }));
    }
  };
    ...
    <div>
      ...
      <div className="border-b-2 border-ededClr pb-5 pt-3 bigScreen:hidden">
        <div className="flex gap-x-[25px]">
          ...
          <div className="border-b-2 pb-[15px] border-ededClr desktop:border-none ">
            ...
            <p className="text-clr333 desktop:text-[20px] font-medium">
              Количество П :{" "}
              <span className="text-clr333 font-bold">{count}</span>
            </p>
            ...
            <div className="flex gap-3 justify-center w-[110px] items-center rounded-md bg-ededClr py-1.5">
              ...
              <strong className="text-clr333">{userCount}</strong>
              <button
        ...
        <div className="flex icons justify-end pt-[20px] gap-4">
          {/* Heart Btn */}
          <button className="flex gap-1 items-center border-[1px] border-clr999 px-1 py-1 rounded-lg">
            <BasketHeartIcon />
      ...
      <div className="hidden bigScreen:inline-block">
        <div className="flex w-[850px] border-b-2 border-ededClr pb-6">
          <img className="w-[170px] h-auto" src={img} alt="img" />

          <div className="w-full">
            {/* Title and Price for Desktop */}
            <div className="flex items-center justify-between">
              <h1 className="text-clr333 font-bold text-[26px]">{title}</h1>
              <p className="text-clr333 font-bold text-[22px]">{price} Сум</p>
            </div>
